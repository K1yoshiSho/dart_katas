...
class Population {
  ...
  Population _newPopulationFromThisPopulation(List<Individual> individuals) => Population(
  ...
  Population naturalSelectionWithDiversity({
    ...
    final List<Individual> selectedIndividuals = _individuals.sublist(0, finalLength);
  ...
  int _retainLength(double retainPercentage) => (length * retainPercentage).toInt();

  bool _randomSelectBiggerThanNextDouble(double randomSelect) => randomSelect > randomGenerator.nextDouble();
  ...
  bool _mutationPercentageBiggerThanNextDouble(double mutationPercentage) => mutationPercentage > randomGenerator.nextDouble();
  int _positionToMutate(Individual individual) => randomGenerator.nextInt(individual.values.length);
  ...
  Individual _newIndividualFromValues(List<double> values) =>
      Individual(IndividualParams(values: values, fitnessFunction: _individuals[0].fitnessFunction));
  ...
  Population crossover({required int desiredLength}) {
    ...
    return _newPopulationFromThisPopulation(<Individual>[..._individuals, ...children]);
  }

  int _childrenLength(int desiredLength) => desiredLength - length;
  int _generateParentRandomIndex() => randomGenerator.nextInt(_individuals.length);
  bool _maleIsNotFemale(int maleIndex, int femaleIndex) => maleIndex != femaleIndex;
